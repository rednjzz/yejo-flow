...
7. **계약서 파일 첨부** — 체결된 계약서 원본(PDF/스캔본)을 첨부할 수 있어야 함. 변경계약 시 변경계약서도 별도 첨부.
...
- `supply_amount` (integer, null: false) — 공급가액
- `vat_amount` (integer, null: false) — 부가가치세
...
- `[contract_id, term_type, seq]` 조합은 유니크해야 한다
...
- `has_many_attached :documents` — 계약서 원본, 특기사항 첨부 등 복수 파일 첨부 가능
- 허용 파일 형식: PDF, JPG, PNG (계약서 스캔본/원본)
- 파일 크기 제한: 개별 파일 최대 20MB
- 유효성 검사: `validate :validate_document_content_type_and_size` (커스텀 검증)

### 4단계: Contract 모델 업데이트

- `has_many :contract_payment_terms, dependent: :destroy` 연관관계 추가
- `has_many_attached :documents` 추가
...
변경계약 관련:
- `change_amount` 검증 추가 — contract_type이 "change"일 때 필수, 원도급의 contract_amount와 비교하여 증감액 = 변경후금액 - 원도급금액
